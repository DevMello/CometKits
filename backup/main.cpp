...
void loop() {
  ...
  accAngleX = (atan(imu.AccY / sqrt(pow(imu.AccX, 2) + pow(imu.AccZ, 2))) * 180 / PI) - AccErrorX; //AccErrorX is calculated in the calculateError() function
  accAngleY = (atan(-1 * imu.AccX / sqrt(pow(imu.AccY, 2) + pow(imu.AccZ, 2))) * 180 / PI) - AccErrorY;
  ...
  imu.GyroX -= GyroErrorX; //GyroErrorX is calculated in the calculateError() function
  imu.GyroY -= GyroErrorY;
  imu.GyroZ -= GyroErrorZ;
  // Currently the raw values are in degrees per seconds, deg/s, so we need to multiply by sendonds (s) to get the angle in degrees
  gyroAngleX += imu.GyroX * elapsedTime; // deg/s * s = deg
  gyroAngleY += imu.GyroY * elapsedTime;
  yaw += imu.GyroZ * elapsedTime;
...
void controlSpeed (){//this function is called by driving ()
  ...
  if (round(targetGyroX - imu.GyroX) == 0){
    ;
  } else if (targetGyroX > imu.GyroX){
...
void rotate (){//called by void loop(), which isDriving = false
  ...
  if (abs(deltaAngle) <= 1){
    ...
    if (round(targetGyroX - abs(imu.GyroX)) == 0){
      ;
    } else if (targetGyroX > abs(imu.GyroX)){
...
void calculateError() {
  ...
  while (c < 200) {
    imu.readAcceleration();
    // Sum all readings
    AccErrorX += (atan((imu.AccY) / sqrt(pow((imu.AccX), 2) + pow((imu.AccZ), 2))) * 180 / PI);
    AccErrorY += (atan(-1 * (imu.AccX) / sqrt(pow((imu.AccY), 2) + pow((imu.AccZ), 2))) * 180 / PI);
    c++;
  }
  ...
  while (c < 200) {
    imu.readGyro();
    // Sum all readings
    GyroErrorX += imu.GyroX;
    GyroErrorY += imu.GyroY;
    GyroErrorZ += imu.GyroZ;
    c++;
  }
